...
#  Parse the file name. It can be any one of the level 1b or level 2 file formats.
    ...
    m = re.search( "([a-zA-Z0-9]{6})_([a-zA-Z0-9]+)\.(\d{4})\.(\d{3})\.(\d{2})\.(\d{2})\.(\w\d{2})\_(\S+)\_", tail )
...
#----------------------------------------
    ...
    if input_phs_type == "1":
        ...
        if 'startTime' in ncattrs:
            ...
            m = re.search( "(\d{4})-(\d{2})-(\d{2})\w+(\d{2}):(\d{2}):([0-9.]+)", units )
            if m:
...
def level2a2aws( atmPrf_file, level2a_file, mission, transmitter, receiver,
    ...
    centerOfCurvature_eci = TEME( x=curv[0], y=curv[1], z=curv[2],
            representation_type='cartesian', obstime=cal.isoformat() )
    centerOfCurvature_ecf = centerOfCurvature_eci.transform_to( ITRS( obstime=cal.isoformat() ) )
    c = centerOfCurvature_ecf.cartesian
    centerOfCurvature = np.array( [ c.x, c.y, c.z ] )
...
def level2b2aws( wetPrf_file, level2b_file, mission, transmitter, receiver,
    ...
    if "date" in ncattrs:
        ...
        regex_search = re.search("(\d{4})\-(\d{2})\-(\d{2})\_(\d{2})\:(\d{2})\:(\d{2})\.", fileDate)
    ...
    m = re.search( "^([-.0-9]+)", str( d.getncattr( "lon" ) ) )
    if m:
    ...
    m = re.search( "^([-.0-9]+)", str( d.getncattr( "lat" ) ) )
    if m:
