...
import os
import numpy as np
from astropy.coordinates import TEME, ITRS, ICRS, SkyCoord
...
#  Fixed location on the Earth.
    ...
    if ECI is ICRS: 
        ...
        ECFx = ITRS( x=1.0, y=0.0, z=0.0, representation_type="cartesian", obstime=obstime0 )
        ECI_of_ECFx_at_e0 = normalize( cartesian( ECFx.transform_to( ECI() ) ) )

        ECFx = ITRS( x=1.0, y=0.0, z=0.0, representation_type="cartesian", obstime=obstime1 )
        ECI_of_ECFx_at_e1 = normalize( cartesian( ECFx.transform_to( ECI() ) ) )

        ECFx = ITRS( x=1.0, y=0.0, z=0.0, representation_type="cartesian", obstime=obstime2 )
        ECI_of_ECFx_at_e2 = normalize( cartesian( ECFx.transform_to( ECI() ) ) )

    else: 

        ECFx = ITRS( x=1.0, y=0.0, z=0.0, representation_type="cartesian", obstime=obstime0 )
        ECI_of_ECFx_at_e0 = normalize( cartesian( ECFx.transform_to( ECI( obstime=obstime0 ) ) ) )

        ECFx = ITRS( x=1.0, y=0.0, z=0.0, representation_type="cartesian", obstime=obstime1 )
        ECI_of_ECFx_at_e1 = normalize( cartesian( ECFx.transform_to( ECI( obstime=obstime1 ) ) ) )

        ECFx = ITRS( x=1.0, y=0.0, z=0.0, representation_type="cartesian", obstime=obstime2 )
        ECI_of_ECFx_at_e2 = normalize( cartesian( ECFx.transform_to( ECI( obstime=obstime2 ) ) ) )
...
#  Define pole direction in ECI direction-coordinates.
    ...
    ECIpole = ECI( x=pole[0], y=pole[1], z=pole[2], representation_type="cartesian" )

#  Pole direction in ECF direction-coordintes.

    ECFpole = ECIpole.transform_to( ITRS( obstime=e0.calendar("utc").isoformat() ) )
