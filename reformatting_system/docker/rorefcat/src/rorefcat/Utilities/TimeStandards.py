...
#  Convert tai to gps and utc time. UTC requires interpretation of leap seconds.
        ...
        if tai is not None:
            if not isinstance(tai,t1900) and not isinstance(tai,Calendar):
                raise TimeError( "InvalidArguments", "tai must be an instance of class t1900" )
            if isinstance(tai,t1900): self.tai = tai
            if isinstance(tai,Calendar): self.tai = tai.t1900
...
#  Convert gps to tai and utc time. UTC requires interpretation of leap seconds.
        ...
        if gps is not None:
            ...
            if isinstance(gps,t1900): 
                ...
                if isinstance(gps,Calendar): 
                    self.gps = gps.t1900
                else: 
                    try: 
                        self.gps = self.gps0 + float( gps )
                    except: 
                        raise TimeError( "InvalidArguments", "gps must be an instance of class t1900, class Calendar, or function as a float" )
...
#  leap seconds, the conversions are ill defined.
        ...
        if utc is not None:
            if not isinstance(utc,t1900) and not isinstance(utc,Calendar):
                raise TimeError( "InvalidArguments", "utc must be an instance of class t1900" )
            if isinstance(utc,t1900): self.utc = utc
            if isinstance(utc,Calendar): self.utc = utc.t1900
            for leapseconddatum in self.leapseconddata:
...
Compute the Julian day.
        ...
        a = int( year / 100 )
        b = 2 - a + int(a/4)
        JDF = int( 365.25 * ( year + 4716 ) ) + int( 30.6001 * ( month + 1 ) ) \
            + cal.day + b - 1524.5 + cal.hour/24.0 \
            + cal.minute/1440.0 + cal.second/86400.0
...
#  Retain only the lines with useful leap-second data.
    ...
    lines = [ line.strip() for line in alllines if line != "" and not re.search( r"^;", line ) ]
